...
const p=PromptTemplate.fromTemplate(ppt3);
const m = createModel({
...
async function executeStep(state, config) {
  ...
  const p1 = pastSteps ? await PromptTemplate.fromTemplate('You have currently done the follow steps:{pastSteps}.\n\n').format({
    pastSteps: pastSteps
      .map(([step, result]) => `${step}: ${result}`)
      .join("\n")
  }):'';
  const p2 = await PromptTemplate.fromTemplate('You current task:{task}').format({
    task
  });
  console.log(p1+p2)
  const input = {
    messages: [new HumanMessage(p1+p2)],
  };
  const { messages } = await agentExecutor.invoke(input, config);
  console.dir(messages[messages.length - 1].content.toString());
  return {
